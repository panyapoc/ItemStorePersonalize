...
export class RecommendationList extends React.Component<RouteComponentProps<RecommendationListProps> & RecommendationListProps, RecommendationListState>{
    constructor(props: RouteComponentProps<RecommendationListProps> & RecommendationListProps) {
        super(props);
      ...
      if (this.props.match.params.searchid && this.props.match.params.searchid.length>0)
      {
    ...
    // Get the data
        ...
        .then(data=>{
           if (!fetchLess)
            ...
            this.setState({isLoading:false,items:data.slice(0,10)});
        });
      }

      static getDerivedStateFromProps(newProps:RouteComponentProps<RecommendationListProps> & RecommendationListProps, prevState:RecommendationListState) {
      ...
      componentDidUpdate(prevProps:RouteComponentProps<RecommendationListProps> & RecommendationListProps, prevState:RecommendationListState) {
      ...
      createTable = () => {
        ...
        for (let i=0;i<listItems.length;i++){
            ...
            children.push(<td key={product.asin}> <ProductRow uid={this.props.userId} key={product.asin} title={product.title} imUrl={product.imUrl} productId={product.asin}></ProductRow></td>);
            ++currentCol;
      ...
      render(){
        ...
        currentClassName="itemsForUser";
          ...
          return(
              <div className={currentClassName}><table className={currentClassName}><tbody>{this.createTable()}</tbody></table></div>
          )
      }
